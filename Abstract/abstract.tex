...
delays by introducing a minimum latency between action execution and the effective 
date at which action execution has been decided, 
and \emph{(ii)} the effect of hardware imperfections, more precisely clock 
drifts, on systems execution by breaking the clock synchrony hypothesis, often adopted during 
...
implementation mechanisms, and thus reduce the gap between the modeling and the concrete
...
to preserve some desired properties and achieve a \say{similar} execution scenario, faithful
